  ...
  static pluralizeEntityList (list, prop = 'name', formatted = true) {
    ...
    if (formatted) {
      ...
      for (const print of toPrint) {
        let temp = sprintf(`%${print.length + 2}s`, print)
      ...
      const checkGrammarSource = (property, fromDefinition = true, parentProp) => {
        ...
        // if the grammar property was set on the entity's definition
          ...
          // otherwise, if the grammar property was imported from metadatabase
            ...
            for (const grammarProperty of entity.metadata.grammar[property]) {
              if (grammarProperty.substr(0, 1) === '$') {
                entity.metadata.grammar[grammarProperty] = [...entity.__manager.grammar[grammarProperty.slice(1)]]

                entity.metadata.grammar[property][entity.metadata.grammar[property].indexOf(grammarProperty)] = `#${grammarProperty}#`
              }
            }
          }
        }
